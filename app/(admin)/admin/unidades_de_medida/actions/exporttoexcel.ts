export default async function ExportUnidadesToExcel(
  ...
  await setData(worksheet, unidades);
...
function setHeaders(worksheet: any) {
  ...
  worksheet.getCell('B1').value = 'Unidade de Medida';
  setHeaderStyle(worksheet.getCell('B1'));
  worksheet.autoFilter = 'A1:B1';
}

const setData = (worksheet: any, unidades: UnidadeDeMedida[]) => {
  unidades.map((unidade) => {
    worksheet.addRow([unidade.id, unidade?.unidade_medida]);
  });
};
