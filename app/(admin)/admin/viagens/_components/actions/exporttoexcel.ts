...
export default async function ExportViagensToExcel() {
  await fetch('/api/viagem/read/all', {
    method: 'GET',
  })
    ...
    .then(
      async (viagens) => {
        ...
        const worksheet = workbook.addWorksheet('Lista de Viagens');
        ...
        worksheet.getColumn('F').width = 60; //Porto de Origem
        worksheet.getColumn('G').width = 60; //Dias no porto de origem
        worksheet.getColumn('H').width = 60; //Porto de Destino
        worksheet.getColumn('I').width = 60; //Dias no porto de destino
        worksheet.getColumn('J').width = 60; //Nome da Embacacao
        worksheet.getColumn('K').width = 60; //Tipo de Embacacao
        worksheet.getColumn('L').width = 60; //Descrição
        worksheet.getColumn('M').width = 60; //Observações
        worksheet.getColumn('N').width = 60; //Comandante
        worksheet.getColumn('O').width = 60; //Capitão
        worksheet.getColumn('P').width = 60; //Armador
        worksheet.getColumn('Q').width = 60; //Mestre
        worksheet.getColumn('R').width = 60; //Referência Documental
        worksheet.getColumn('S').width = 60; //Passageiros
        worksheet.getColumn('T').width = 60; //Arriba
        worksheet.getColumn('U').width = 60; //Notícias

        await setHeaders(worksheet);
        await setData(worksheet, viagens);
        await saveFile(
          'Export_' + Math.floor(Math.random() * 100000),
          workbook
        );
      },
      (error) => {
...
function setHeaders(worksheet: any) {
  ...
  worksheet.getCell('F1').value = 'Porto de Origem';
  setHeaderStyle(worksheet.getCell('F1'));
  worksheet.getCell('G1').value = 'Dias no porto de origem';
  setHeaderStyle(worksheet.getCell('G1'));
  worksheet.getCell('H1').value = 'Porto de Destino';
  setHeaderStyle(worksheet.getCell('H1'));
  worksheet.getCell('I1').value = 'Dias no porto de destino';
  setHeaderStyle(worksheet.getCell('I1'));
  worksheet.getCell('J1').value = 'Nome da Embacacao';
  setHeaderStyle(worksheet.getCell('J1'));
  worksheet.getCell('K1').value = 'Tipo de Embacacao';
  setHeaderStyle(worksheet.getCell('K1'));
  worksheet.getCell('L1').value = 'Descrição';
  setHeaderStyle(worksheet.getCell('L1'));
  worksheet.getCell('M1').value = 'Observações';
  setHeaderStyle(worksheet.getCell('M1'));
  worksheet.getCell('N1').value = 'Comandante';
  setHeaderStyle(worksheet.getCell('N1'));
  worksheet.getCell('O1').value = 'Capitão';
  setHeaderStyle(worksheet.getCell('O1'));
  worksheet.getCell('P1').value = 'Armador';
  setHeaderStyle(worksheet.getCell('P1'));
  worksheet.getCell('Q1').value = 'Mestre';
  setHeaderStyle(worksheet.getCell('Q1'));
  worksheet.getCell('R1').value = 'Referência Documental';
  setHeaderStyle(worksheet.getCell('R1'));
  worksheet.getCell('S1').value = 'Passageiros';
  setHeaderStyle(worksheet.getCell('S1'));
  worksheet.getCell('T1').value = 'Arriba';
  ...
  viagens.map((viagem) => {
    worksheet.addRow([
      ...
      viagem?.passageiro
        ?.map(
          (passageiro) =>
            passageiro.tipo_passageiro?.tipo + ': ' + passageiro?.total
        )
        .join(', ') || '',
      viagem?.arriba?.map((arriba) => arriba.observacoes).join(', ') || '',
