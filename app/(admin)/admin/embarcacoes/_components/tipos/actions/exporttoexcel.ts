export default async function ExportTiposToExcel(tipos: TipoEmbarcacao[]) {
...
function setHeaders(worksheet: any) {
  ...
  worksheet.getCell("B1").value = "Tipo";
  setHeaderStyle(worksheet.getCell("B1"));
  worksheet.autoFilter = "A1:B1";
}

const setData = (worksheet: any, tipos: TipoEmbarcacao[]) => {
  tipos.map((tipo) => {
    worksheet.addRow([tipo.id, tipo?.tipo]);
  });
};
