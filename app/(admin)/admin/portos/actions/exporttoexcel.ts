export default async function ExportPortosToExcel(portos: Porto[]) {
  ...
  await setData(worksheet, portos);
...
function setHeaders(worksheet: any) {
  ...
  worksheet.getCell('C1').value = 'País';
  setHeaderStyle(worksheet.getCell('C1'));
  worksheet.autoFilter = 'A1:C1';
}

const setData = (worksheet: any, portos: Porto[]) => {
  portos.map((porto) => {
    worksheet.addRow([porto.id, porto?.nome, porto?.pais?.pais]);
  });
};
